...
model = Sequential()
model.add(Dense(1024, input_shape = (13, ),activation='sigmoid'))
...
es = EarlyStopping(monitor= 'val_loss', mode = 'auto', patience= 3000, verbose = 0, restore_best_weights= True)
start_time = time.time()
hist = model.fit(x_train, y_train, epochs = 20000, batch_size= 4000, validation_split= 0.35, verbose=2, callbacks= [es])
end_time = time.time()
