...
x_train, x_test, y_train, y_test = train_test_split(x, y, train_size= 0.7, shuffle= True, random_state= 4882, stratify= y)
...
model.fit(x_train, y_train, epochs = 1500, batch_size= 15, validation_split= 0.4, verbose=2, callbacks= [es])
end_time = time.time()
...
submission_csv.to_csv(path+f"submission_{save_time}{acc}.csv", index=False)
