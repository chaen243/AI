...
train_csv['대출등급'] = le.transform(train_csv['대출등급'])
######## 결측치확인
# print(test_csv.isnull().sum()) #없음.
# print(train_csv.isnull().sum()) #없음.
...
model = Sequential()
model.add(Dense(10, input_dim=14, activation='swish'))
...
hist = model.fit(x_train, y_train, callbacks=[es], epochs= 100, batch_size = 1050, validation_split= 0.2, verbose=2)
end_time = time.time()
