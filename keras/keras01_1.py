...
from tensorflow.keras.models import Sequential # from안에 imoort를 땡겨옴 (순차적으로 땡겨옴) #warning 경고는 하지만 실행 가능, error는 실행불가
...
#2. 모델구성
model = Sequential()    #모델의 변수명을 정의. Sequential=순차적모델을 만듬
...
model.fit(x, y, epochs=8000) #fit= 훈련 훈련양 조절 필수. 과하게 훈련하면 과적합 걸릴 수 있음. # 훈련양 조절로 최적의 웨이트값을 생성

#4. 평가, 예측 (loss값 평가, 정확도/)
loss = model.evaluate(x, y) #생성된 웨이트값을 예측해줌
print("로스 : ", loss) #""안의 글자는 그대로 출력
result = model.predict([4]) #변수는 임의로 지정 가능하지만 가독성을 위해 암묵적으로 약속된 변수 사용.
print("4의 예측값 : ", result)
