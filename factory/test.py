...
# 데이터 위치
IMAGES_PATH = path + 'train_img/'
MASKS_PATH = path +'train_mask/'
...
# print(MASKS_PATH)
# print(mask for mask in str(x_val['train_mask']) )


# train : val 지정 및 generator
images_train = [os.path.join(IMAGES_PATH, image) for image in str(x_tr['train_img']) ]
masks_train = [os.path.join(MASKS_PATH, mask) for mask in str(x_tr['train_mask']) ]

images_validation = [os.path.join(IMAGES_PATH, image) for image in str(x_val['train_img'] )]
masks_validation = [os.path.join(MASKS_PATH, mask) for mask in str(x_val['train_mask']) ]
